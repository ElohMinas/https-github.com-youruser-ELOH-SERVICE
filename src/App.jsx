...
const defaultData = {
  ...
  os: [
    // Exemplo inicial (vazio por padrão)
  ]
};
function loadData() {
...
function SignaturePad({ onSave }) {
  ...
  useEffect(() => {
    ...
    const start = (e) => { drawing.current = true; ctx.beginPath(); const p = getPos(e); ctx.moveTo(p.x, p.y); e.preventDefault(); };
    const move = (e) => { if (!drawing.current) return; const p = getPos(e); ctx.lineTo(p.x, p.y); ctx.stroke(); e.preventDefault(); };
...
function PhotoUploader({ photos = [], onChange }) {
  const handleFiles = (e) => {
    ...
    Promise.all(readers).then(results => onChange([ ...photos, ...results ].slice(0, 5)));
  };
  return (
    <div>
...
function CreateOSForm({ users, onCreate }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    if (!client || !desc) return alert('Preencha cliente e descrição');
    onCreate({ client, address, description: desc, priority, assignedTo });
    setClient(''); setAddress(''); setDesc(''); setPriority('Normal');
  };
  ...
  return (
    <form onSubmit={submit} className="space-y-2">
      ...
      <div className="flex justify-end">
        <button type="submit" className="px-4 py-2" style={{ background: colors.primary, color: '#fff', borderRadius: 8 }}>Criar OS</button>
      </div>
    </form>
  );
}
...
function AdminReports({ osList, users }) {
  ...
  const exportCSV = () => {
    ...
    const csv = rows.map(r => r.map(cell => '"' + String(cell).replace(/"/g,'""') + '"').join(','));
